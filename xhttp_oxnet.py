# xhttp_oxnet.py — OXNET v2.0
# XHTTP transport. Protocol-agnostic: auto-detects VLESS or Trojan from the
# first uplink chunk and applies the correct response framing.
import asyncio
import secrets
import socket
import time
...
async def _open_tcp_from_header(first_chunk: bytes, trojan_pw: str | None = None):
    info = parse_any_header(first_chunk, trojan_pw)
    reader, writer = await asyncio.wait_for(
        asyncio.open_connection(info["address"], info["port"]), timeout=TCP_CONNECT_TIMEOUT
    )
    _tune_socket(writer)
    if info["payload"]:
        writer.write(info["payload"])
        await writer.drain()
    return reader, writer, info["address"], info["port"], info["proto"], info["resp_prefix"]


async def _resolve_or_403(path_key: str) -> tuple[str, dict]:
...
async def _pump_tcp_to_queue(session_id, uuid, reader, down_q, resp_prefix=b""):
    first = True
...
async def _open_tcp_for_session(session_id, uuid, sess, first_chunk):
    async with LINKS_LOCK:
        link = LINKS.get(uuid)
    trojan_pw = trojan_password(link, uuid)
    reader, writer, address, port, proto, resp_prefix = await _open_tcp_from_header(first_chunk, trojan_pw)
    logger.info(f"connect XHTTP[{sess['mode']}] [{session_id[:8]}] {proto} -> {address}:{port}")
    sess["writer"] = writer
    sess["tcp_open"] = True
    sess["resp_prefix"] = resp_prefix
    c = connections.get(sess["conn_id"])
    if c:
        c["transport"] = f"{proto}-xhttp-{sess['mode']}"
    sess["downlink_task"] = asyncio.create_task(
        _pump_tcp_to_queue(session_id, uuid, reader, sess["down_q"], resp_prefix)
    )
...
@router.get("/xhttp-oxnet/{mode}/{path_key}/{session_id}")
async def xhttp_downlink(mode: str, path_key: str, session_id: str, request: Request):
    ensure_reaper()
    if mode not in ("packet-up", "stream-up"):
        raise HTTPException(status_code=404, detail="unknown mode")
    uuid, _link = await _resolve_or_403(path_key)
...
@router.post("/xhttp-oxnet/packet-up/{path_key}/{session_id}/{seq}")
async def packet_up_upload(path_key: str, session_id: str, seq: int, request: Request):
    ensure_reaper()
    uuid, _link = await _resolve_or_403(path_key)
...
@router.post("/xhttp-oxnet/stream-up/{path_key}/{session_id}")
async def stream_up_upload(path_key: str, session_id: str, request: Request):
    ensure_reaper()
    uuid, _link = await _resolve_or_403(path_key)
